    ...
    if (isset($_POST["palavra-chave"]) || isset($_POST["centro-custo"]) || isset($_POST["ordenar-por"])) {
        ...
        switch (true) {
            case !empty($searchKeyWordInput):
                ...
                opcao_aprovada_rejeitada='Rejeitada' AND 
            ...
            case !empty($searchInstitutionInput):
                $selectTable = "SELECT * FROM cotacoes_em_andamento WHERE id='$searchInstitutionInput' AND opcao_aprovada_rejeitada='Rejeitada'";
                break;

            case !empty($orderByInput):
                switch ($orderByInput) {
                    case "numero_veiculo_decrescente":
                        $selectTable = "SELECT * FROM cotacoes_em_andamento WHERE opcao_aprovada_rejeitada='Rejeitada' ORDER BY id DESC";
                        break;

                    case "numero_veiculo_crescente":
                        $selectTable = "SELECT * FROM cotacoes_em_andamento WHERE opcao_aprovada_rejeitada='Rejeitada' ORDER BY id ASC";
                        break;

                    default:
                        $selectTable = "SELECT * FROM cotacoes_em_andamento WHERE opcao_aprovada_rejeitada='Rejeitada' ORDER BY $orderByInput ASC";
                        break;
                }
                break;
        }
